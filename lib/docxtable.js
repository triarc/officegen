...
module.exports = {
  ...
  _getCell: function (val, opts, tblOpts) {
    ...
    if( typeof opts['borders'] != 'undefined' ) {
        cellObj['w:tc']['w:tcPr']['w:tcBorders'] = {};
        var tcBorders = cellObj['w:tc']['w:tcPr']['w:tcBorders'];
        var sides = ['top', 'left', 'bottom', 'right'];
        for( var i = 0;i < sides.length;i++ ) {
            // calculate side config
            var sideName = sides[i];
            var sideTag = `w:${sideName}`;
            var sideCfg = null;
            if( typeof opts['borders']['all'] != 'undefined' ) {
                sideCfg = opts['borders']['all'];
            } else {
                if( typeof opts['borders'][sideName] != 'undefined' ) {
                    sideCfg = opts['borders'][sideName];
                }
            }

            // if side config founded, form styles for current side
            if( sideCfg !== null ) {
                tcBorders[sideTag] = {};
                var tcBorder = tcBorders[sideTag];
                if( sideCfg['val'] === null ) {
                    tcBorder['@w:val'] = 'nil';
                } else {
                    if( sideCfg['val'] ) {
                        tcBorder['@w:val'] = sideCfg['val'];
                    } else {
                        tcBorder['@w:val'] = 'single';
                    }
                    if( sideCfg['size'] ) {
                        tcBorder['@w:sz'] = sideCfg['size'];
                    }
                    if( sideCfg['color'] ) {
                        tcBorder['@w:color'] = sideCfg['color'];
                    }
                }
            }
        }
